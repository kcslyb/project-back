...
public class BaseController {
    ...
    public JSONObject login(@RequestBody JSONObject requestJson) {
        ...
        try {
            ...
            returnData.put("status", ErrorEnum.E_200.getStatus());
            returnData.put("message", ErrorEnum.E_200.getMessage());
        } catch (AuthenticationException e) {
            returnData.put(HttpStatus.BAD_REQUEST.value() + "", HttpStatus.BAD_REQUEST.getReasonPhrase());
        }
        List<TLogger> tLoggers = tLoggerService.queryAllByLimit(new TLogger(), 0, 1);
        if (!CollectionUtil.isEmpty(tLoggers)) {
            TLogger tLogger = tLoggers.get(0);
            tLogger.setLogAccessUserId(LoginInfo.getUserId());
            tLogger.setLogAccessUserName(LoginInfo.getUserName());
            tLoggerService.update(tLogger);
        }
        return returnData;
    }

...
import cn.kcs.user.entity.TRole;
import cn.kcs.user.entity.dto.RolePermission;
...
public class TRoleController {
    ...
    public JSONObject updateRolePermission(TRole role) {
        ...
        if (!CollectionUtils.isEmpty(rolePermissions) && CollectionUtils.isEmpty(rolePermissions.get(0).getPermissions())) {
        ...
        if (!CollectionUtils.isEmpty(role.getPermissionIds())) {
            ...
            for (String permissionId : role.getPermissionIds()) {
                ...
                // 如果父级存在，添加父级权限
                if ("1".equals(tPermissionMenu.getPermtype()) && !list.contains(tPermissionMenu.getPermparent())) {
